...
class HealthGrpcService : HealthGrpc.HealthImplBase() {
    ...
    override fun watch(
        ...
        grpcStreamScope.launch(dispatcher) {
    ...
    companion object {
        ...
        private val dispatcher = newFixedThreadPoolContext(5, "watch-context")
        private val grpcStreamScope = CoroutineScope(dispatcher)
    }
}

private fun sendSingleResponse(
    response: HealthOuterClass.HealthCheckResponse,
    responseObserver: StreamObserver<HealthOuterClass.HealthCheckResponse>?,
) {
    responseObserver?.onNext(response)
...
private fun makeResponse(status: HealthOuterClass.HealthCheckResponse.ServingStatus) =
    HealthOuterClass.HealthCheckResponse.newBuilder().setStatus(
        status).build()
