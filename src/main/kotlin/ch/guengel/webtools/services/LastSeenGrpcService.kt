...
class LastSeenGrpcService(private val lastSeenService: LastSeenService) : LastSeenGrpc.LastSeenImplBase() {
    override fun getLastSeen(request: Lastseen.LastSeenRequest?,
        ...
        try {
            ...
            if (request.since.isBlank()) {
                result = lastSeenService.countOccurrencesSince(request.ip, "1d")
            } else {
                result = lastSeenService.countOccurrencesSince(request.ip, request.since)
            }
            ...
            val exception = Status.fromCode(Status.Code.INVALID_ARGUMENT).withDescription(e.message).withCause(e).asRuntimeException()
            responseObserver?.onError(exception)
        } catch (e: NoSuchElementException) {
            logger.error("Error getting count for IP ${request.ip}", e)
            val exception = Status.fromCode(Status.Code.NOT_FOUND).withDescription("IP not found").withCause(e).asRuntimeException()
            responseObserver?.onError(exception)
        } catch (e: Exception) {
            logger.error("Error getting count for IP ${request.ip}", e)
    ...
    override fun updateLastSeen(request: Lastseen.UpdateLastSeenRequest?,
                                responseObserver: StreamObserver<Lastseen.LastSeenReply>?) {
        if (request == null) {
            val exception = Status.INVALID_ARGUMENT.asRuntimeException()
            responseObserver?.onError(exception)
        ...
        if (request.ip.isBlank()) {
            val exception = Status.fromCode(Status.Code.INVALID_ARGUMENT).withDescription("Require IP Address").asRuntimeException()
            responseObserver?.onError(exception)
        }
